...
A project may declare memorable root-level shortcuts with an optional `aliases` array in `project.json`. Each alias requires a matching `<alias>/index.html` containing an immediate relative HTML redirect and a fallback link to the canonical `projects/<slug>/index.html`; keep the alias outside the project so the project remains portable. Treat aliases as manually maintained and update or remove them whenever the canonical project path changes.
